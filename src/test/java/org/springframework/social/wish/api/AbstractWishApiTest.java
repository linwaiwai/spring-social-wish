...
public class AbstractWishApiTest {
	...
	protected WishTemplate wish;
	protected WishTemplate appFacebook;
	...
	@Before
	public void setup() {
		wish = createWishTemplate();
		mockServer = MockRestServiceServer.createServer(wish.getRestTemplate());
		
		appFacebook = new WishTemplate(APP_ACCESS_TOKEN);
		appFacebookMockServer = MockRestServiceServer.createServer(appFacebook.getRestTemplate());
	}

	protected WishTemplate createWishTemplate() {
		return new WishTemplate(ACCESS_TOKEN, "APP_NAMESPACE", "APP_ID");
	}
	...
	protected String wishUrl(String path) {
		return wish.getBaseGraphApiUrl() + path;
	}
